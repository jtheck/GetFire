...
if(!$t.className.includes("gf_hcmod"))$mControls.style.display="none"});$wrapper.addEventListener('mouseleave',function(e){$respondB.setAttribute("data-id",null);$mControls.style.display="none"});var $form=document.createElement("form");$form.setAttribute("autocomplete","off");var $input=document.createElement("textarea");$input.setAttribute('placeholder',"compose your message here");$input.id="gf_message_entry";$input.classList.add("gf_scroll");var $submit=document.createElement("input");$submit.setAttribute('type',"submit");$submit.setAttribute('value',"Send");$submit.id="gf_submit";$submit.onclick=function(){GETFIRE.postMessage({content:$input.value,topic:GETFIRE.id,color:GETFIRE.uColor,user:GETFIRE.user,name:GETFIRE.uName,response_to:$responding.getAttribute("data-id")});return!1};$form.append($input);$form.append($submit);GETFIRE.$topic.append($form);var pubk="pub-c-1792f899-2843-41fa-bb31-28d7190cee7a";var subk="sub-c-8835c7da-7a67-11e4-b197-02ee2ddab7fe";GETFIRE.pn=new PubNub({subscribeKey:subk,publishKey:pubk,ssl:!0});if(document.cookie.indexOf("gfa_pn_uuid=")>=0)GETFIRE.uuid=getCookie("gfa_pn_uuid");else GETFIRE.uuid=setCookie("gfa_pn_uuid",Math.random().toString(36).substring(2,15)+"GF_applet"+Math.random().toString(36).substring(2,15));GETFIRE.pn.uuid=GETFIRE.uuid;GETFIRE.pn.addListener({status:function(s){},message:function(m){renderMessage(m.message.content)}});if(ssl)subscribe();else $pContent.innerHTML=GETFIRE.$messages.innerHTML="<BR><BR>SSL required.";gfResize();if(startOpen){GETFIRE.$topic.style.display="block";GETFIRE.$preview.style.display="none";$icon.style.display="none"}
var timeagoNodes=document.querySelectorAll('.timeago');ta.render(timeagoNodes);function subscribe(){var xhr=window.XMLHttpRequest?new XMLHttpRequest():new ActiveXObject('Microsoft.XMLHTTP');var params=JSON.stringify({name:name});xhr.open("post",uri+"api/v1/index",!0);xhr.setRequestHeader('Content-Type','application/json; charset=utf-8');xhr.onload=function(){if(xhr.status===200){var data=JSON.parse(xhr.responseText).data;if(data){GETFIRE.id=data.hashish;GETFIRE.pn.subscribe({channels:["ft-"+GETFIRE.id]});name=$head.innerHTML=$pName.innerHTML=data.name;$ideal=newDiv({id:"gf_ideal",content:data.summary||"No topic ideal."});$settings.append($ideal);var urlName=name.replace(/ /g,"_");$tshare=document.createElement('a');$fshare=document.createElement('a');$tshare.innerHTML="t";$fshare.innerHTML="f";$tshare.id="gf_tshare";$fshare.id="gf_fshare";$tshare.title="Share on Twitter!";$fshare.title="Share on facebook!";$tshare.href="https://twitter.com/home?status=GetFire.net/"+urlName;$fshare.href="https://www.facebook.com/sharer/sharer.php?u=GetFire.net/"+urlName;$settings.append($tshare);$settings.append($fshare);var watching=JSON.parse(xhr.responseText).watching;$watching=newDiv({id:"gf_watching",content:watching+" watching"});$settings.append($watching);var messages=JSON.parse(xhr.responseText).messages;for(var i=0;i<messages.length;i++){renderMessage(messages[i])}
GETFIRE.ready=!0}else{GETFIRE.$topic.style.display="none";GETFIRE.$preview.style.display="block";$icon.style.display="none";$pContent.innerHTML=GETFIRE.$messages.innerHTML="<BR><BR>Topic not found!";console.log("GetFire subscribe failed!")}}else{console.log('Request failed: '+xhr.status)}};xhr.send(params);return!0};function publish(data){GETFIRE.pn.publish({channel:"ft-"+GETFIRE.id,message:{type:"new_message",content:data,profile:GETFIRE.user}},function(status,response){if(status.error){}else{$input.value=""}})};GETFIRE.postMessage=function(msg){if(!GETFIRE.ready)return!1;var xhr=window.XMLHttpRequest?new XMLHttpRequest():new ActiveXObject('Microsoft.XMLHTTP');var params=JSON.stringify({content:msg.content,topic_id:msg.topic,color:msg.color,user_id:msg.user,name:msg.name,response_to:msg.response_to});xhr.open("post",uri+"api/v1/message",!0);xhr.setRequestHeader('Content-Type','application/json; charset=utf-8');xhr.onload=function(){if(xhr.status===200){var data=JSON.parse(xhr.responseText).data;if(data&&data.content!=""){data=JSON.parse(data);publish(data)}}else{console.log('Request failed: '+xhr.status)}};xhr.send(params)};GETFIRE.rules=function(){alert("be a baws<br>dont be traj");return!0};function renderMessage(m){if(!m)return!1;var mContent=parseMessage(m.content);var $tMsg=newDiv({className:"gf_msg gf_hcmod",content:mContent});$tMsg.setAttribute("data-time",m.created_at);$tMsg.setAttribute("data-id",m.user_id);$tMsg.setAttribute("data-name",m.name);$tMsg.setAttribute("data-msg-id",m.hashish);var $name=document.createElement("span");$name.classList.add("gf_mName");$name.classList.add("gf_hcmod");$name.style.color=m.color;renderPreview(mContent);if(GETFIRE.$topic.style.display=="none"&&GETFIRE.$preview.style.display=="none"){GETFIRE.$preview.style.display="block";$icon.style.display="none"}
if(m.response_to=="null"){$name.innerHTML=m.name+"‧ ";$tMsg.prepend($name);GETFIRE.$messages.append($tMsg)}else{if(m.response_to=="ADMIN"){var $am=newDiv({className:"gf_am",content:mContent});GETFIRE.$messages.append($am)}else{$name.innerHTML=" ‧"+m.name;$tMsg.append($name);$tMsg.classList.add("gf_mResponse");$tMsg.setAttribute("data-response-id",m.response_to);GETFIRE.$messages.append($tMsg)}}
GETFIRE.$messages.scrollTop=GETFIRE.$messages.scrollHeight;return!0};function renderPreview(m){var pCount=$pContent.getElementsByClassName("gf_pMsg").length;if(pCount==0)$pContent.innerHTML="";var $newMsg=newDiv({className:"gf_pMsg",content:m});$pContent.append($newMsg);if(pCount>4)$pContent.getElementsByClassName("gf_pMsg")[0].remove();return!0};function newDiv(div){var $div=document.createElement('div');$div.id=div.id||"";$div.className=div.className||"";$div.setAttribute('title',(div.title||""));$div.innerHTML=div.content||"";return $div};function parseMessage(m){m=m.replace(/</g,"&lt;").replace(/>/g,"&gt;");var replacedText,replacePattern1,replacePattern2;replacePattern1=/(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim;replacedText=m.replace(replacePattern1,'<a href="$1" target="_blank" rel="noopener">$1</a>');replacePattern2=/(^|[^\/])(www\.[\S]+(\b|$))/gim;replacedText=replacedText.replace(replacePattern2,'$1<a href="http://$2" target="_blank" rel="noopener">$2</a>');return replacedText};function gfClick(e){var path=e.path;if(path.includes($wrapper)||path.includes($popup)){if(!path.includes($settings)&&!path.includes($card))$settings.style.display="none";var foundPopup=!1;for(var i=0;i<path.length;i++)if(path[i].classList&&path[i].classList.value.includes("gf_mName"))foundPopup=path[i];if(foundPopup==!1){$popup.style.display="none"}else{var $tMsg=foundPopup.parentElement;var px=Math.max(1,Math.min(e.clientX-75,window.innerWidth-156));var py=Math.max(4,(e.clientY-75));var id=$tMsg.getAttribute("data-id");var iurl="https://s3.amazonaws.com/getfire-paperclip-"+env+"/avatars/"+id+"/half.jpg";if(id.slice(0,4)=="NOID")iurl="";var name=$tMsg.getAttribute("data-name");$popup.style.display="block";$popup.style.borderColor=foundPopup.style.color;$popup.style.left=px;$popup.style.top=py;$popup.setAttribute("data-id",id);$popup.setAttribute("data-name",name);$popup.style.backgroundImage="url("+iurl+")"}
if(!path.includes($responding)&&!path.includes($input)&&!path.includes($submit)&&!path.includes($respondB)){$responding.style.display="none";$responding.setAttribute("data-id",null)}}else{$popup.style.display="none";if($icon.style.display=="none"){GETFIRE.$topic.style.display="none";GETFIRE.$preview.style.display="block"}}};function gfResize(){var width=Math.min(Math.max(win.innerWidth/3,316),win.innerWidth);var height=win.innerHeight;var halfHeight=height<316?height:316;GETFIRE.$topic.style.width=width;GETFIRE.$topic.style.height=fullHeight?height:halfHeight;return!0};function getCookie(name){match=document.cookie.match(new RegExp(name+'=([^;]+)'));if(match)return match[1];else return!1};function setCookie(name,value){var cookie=name+"="+encodeURIComponent(value);cookie+="; max-age="+(4*365*24*60*60);cookie+="; path=/";document.cookie=cookie;return!0};function makeColor(){var hexletters='0123456789ABCDEF',color='#';for(var i=0;i<6;i++)color+=hexletters[Math.floor(Math.random()*16)];return color};function makeID(){var rando="";var possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(var i=0;i<8;i++)
